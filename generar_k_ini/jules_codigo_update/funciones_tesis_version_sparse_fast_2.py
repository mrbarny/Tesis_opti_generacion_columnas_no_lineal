...
def generar_canonicos_filtro(indices_features, n_features, n_samples):
    """
    Genera una lista de elemetos de K de vectores canónicos usando la función base existente.
    """
    nuevos_rayos = []
...
def _solve_pricing_base(X, y, grad_w_vector, C, PRICING_PARAMS, M_box, 
    ...
    # Usamos enteros para evitar errores de atributo, pero sin hacks de archivos.
    # 1 = ON, 0 = OFF
    safe_params = {
    ...
    try:
        ...
        print(f"[pricing] SolverError: {e}")
        return (None, None, None, 0.0)

    st = (prob.status or "").lower()

    # --- 4. VERIFICACIÓN DEL PARCHE ---
    
    # CASO A: Óptimo normal
    if st == 'optimal':
        return _pack_solution(w.value, b.value, xi.value, prob.value)

    # CASO B: No Acotado / Infactible (Dual)
    # Aquí es donde probaremos si tu cambio en mosek_conif.py funcionó.
    elif 'unbounded' in st or 'infeasible' in st:
        print(f"[pricing] Status detectado: {st}")
        ...
        if w.value is not None:
            ...
            if norm > 1e-9:
                ...
                return _pack_solution(w.value * scale, b.value * scale, xi.value * scale, -np.inf)
            else:
                print("[pricing] Advertencia: El rayo recuperado es nulo (norma ~ 0).")
        else:
            print(f"[pricing] FALLO: Status es {st} pero w.value sigue siendo None.")
            print("[pricing] El parche en mosek_conif.py no se activó o no encontró la solución en memoria.")
            
            
            
            

    return (None, None, None, 0.0) #SI MUERE RETORNA NONES Y 0. INTERESANTE. 
...
def solve_pricing_problem_caja(X, y, grad_w, K=None, C=1.0, PRICING_PARAMS={}, M_box=1e6,add_stabilization_cut=False, warm_start=True,verbose=True):
    ...
    try:
        res = _solve_pricing_base(X, y, grad_w, C, PRICING_PARAMS, M_box, warm_start=warm_start,verbose=verbose,add_stabilization_cut=add_stabilization_cut)
    ...
    params_rescue["MSK_IPAR_OPTIMIZER"] = 2  # Simplex Dual #1 es primal #0 es descenso, rapido pero no tan bueno. 
    params_rescue["MSK_IPAR_PRESOLVE_USE"] = 0 # Sin Presolve
    
    try:
        # AQUÍ ESTABA EL PROBLEMA: Si esto lanza error, el script moría.
        # Ahora lo capturamos.
        res_retry = _solve_pricing_base(X, y, grad_w, C, params_rescue, M_box, warm_start=warm_start,verbose=verbose, add_stabilization_cut=add_stabilization_cut)
...
class GeneracionColumnasDW:
    ...
    def ingresar_parametros(self, C, M, K_ini_points, K_ini_rays=None, 
                            tipo="convexo", 
                            pricing=solve_pricing_problem_caja, 
                            gradient_strategy="full_gradient", pricing_acceleration=False,pricing_acceleration_cota=False,tijonov=True):
        ...
        self.pricing_acceleration_cota = pricing_acceleration_cota #ya no se usa
