...
def plot_convergence_paper(history, conic_opt=None, title="Convergencia Primal-Dual", filename=None, log_scale=False):
    ...
    if filename:
        try:
            ...
            print(f"Advertencia al guardar gráfico: {e}")
            try:
                ax.set_yscale('linear')
                plt.savefig(filename, dpi=300)
            except Exception:
...
def plot_convergence_metrics(history, conic_opt, dataset_size, use_filter_iters=None, filename=None):
    """
...
def plot_convergence_split(history, conic_opt=None, title="Convergencia Primal-Dual (Separada)", filename=None, use_log_scale=False):
    ...
    if filename:
        try:
            ...
            print(f"Advertencia al guardar gráfico split: {e}")
            try:
                ax2.set_yscale('linear')
                plt.savefig(filename, dpi=300)
            except Exception:
...
def plot_heatmap_memoria_robusto(historial_dict, tipo="Theta", log_scale=False, filename=None):
    """
