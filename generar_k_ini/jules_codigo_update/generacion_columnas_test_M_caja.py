...
# In[5]:
# 2. Extracción de Métricas para Graficar

valores_objetivo = np.array(historico_resultados["opt_vals"])
tiempos_master = np.array(gen_col.time_master)
tiempos_pricing = np.array(gen_col.time_pricing)
iteraciones = np.arange(len(valores_objetivo))

# Crear paneles de visualización científica
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 5))

# --- Gráfica 1: Evolución del Upper Bound (Función Objetivo del Master) ---
ax1.plot(iteraciones, valores_objetivo, marker='o', color='navy', linestyle='-', linewidth=2, label='Master Obj (UB)')
ax1.set_title("Convergencia del Problema Maestro Dantzig-Wolfe", fontsize=12, fontweight='bold')
ax1.set_xlabel("Iteración", fontsize=10)
ax1.set_ylabel("Valor Función Objetivo", fontsize=10)
ax1.grid(True, linestyle='--', alpha=0.6)
ax1.legend()

# --- Gráfica 2: Análisis de Tiempos de Cómputo (Master vs Pricing) ---
# Graficamos los tiempos por separado para evaluar el impacto de Gurobi Matricial
ax2.plot(range(len(tiempos_master)), tiempos_master, marker='s', color='darkorange', linestyle='--', label='Tiempo Master (Mosek)')
ax2.plot(range(len(tiempos_pricing)), tiempos_pricing, marker='^', color='forestgreen', linestyle='--', label='Tiempo Pricing (Gurobi/Screening)')
ax2.set_title("Perfil de Tiempo de Ejecución por Componente", fontsize=12, fontweight='bold')
...
# 3. Guardado opcional de datos para anexos de tu documento escrito
# np.save('evolucion_objetivo.npy', valores_objetivo)
...
def graficar_memoria_columnas(memoria_dict, titulo="Evolución de Coeficientes", es_log=False):
    """
    Grafica la evolución de los coeficientes (theta o mu) por columna.
    memoria_dict: memoria_theta o memoria_mu de la clase GeneracionColumnasDW_2
    """
    # 1. Preparar matriz
    n_cols = len(memoria_dict)
    max_iter = max(len(v) for v in memoria_dict.values())
    matriz = np.full((n_cols, max_iter), np.nan)
    
    claves_ordenadas = sorted(memoria_dict.keys(), key=lambda x: int(x.split('_')[1]))
    
    for i, k in enumerate(claves_ordenadas):
        vals = np.array(memoria_dict[k], dtype=float)
        if es_log:
            # log10 de valor absoluto clip para evitar log(0)
            matriz[i, :len(vals)] = np.log10(np.clip(np.abs(vals), 1e-12, None))
        else:
            matriz[i, :len(vals)] = vals

    # 2. Plotting
    fig, ax = plt.subplots(figsize=(15, 10))
    im = ax.imshow(matriz, cmap="viridis" if not es_log else "magma", aspect="auto", origin="lower")
    plt.colorbar(im, label="Log₁₀(|θ|)" if es_log else "Valor")
    ax.set_xlabel("Iteración")
    ax.set_ylabel("ID Columna")
    ax.set_title(titulo)
    plt.tight_layout()
    plt.show()

# Uso sugerido:
graficar_memoria_columnas(gen_col.memoria_theta, titulo="Evolución de Theta (Puntos)")
graficar_memoria_columnas(gen_col.memoria_mu, titulo="Evolución de Mu (Rayos)", es_log=True)

def graficar_columnas(gen_col, usar_permanente=False, log_scale=True, M=10000):
    """
    memoria: dict con claves k_0, k_1... y listas de valores.
    usar_permanente: Si True, usa memoria_permanente (muestra eliminadas).
                     Si False, usa memoria_theta (solo activas).
    """
    if usar_permanente:
        memoria = reconstruir_historial_completo(gen_col, M=M)
    else:
        # Combinamos ambas memorias en una sola para el plot
        memoria = {**gen_col.memoria_theta, **gen_col.memoria_mu}
    
    # 1. Preparar datos
    n_cols = len(memoria)
    max_iter = max(len(v) for v in memoria.values())
    matriz = np.full((n_cols, max_iter), np.nan)
    claves = sorted(memoria.keys(), key=lambda x: int(x.split("_")[1]))
    
    for i, k in enumerate(claves):
        vals = np.array(memoria[k], dtype=float)
        # Transformación logarítmica si es necesario
        if log_scale:
            # Marcamos -M para que el imshow lo ignore o lo pinte de otro color
            mask = (vals != -M)
            matriz[i, :len(vals)] = np.where(mask, np.log10(np.clip(np.abs(vals), 1e-12, None)), np.nan)
        else:
            matriz[i, :len(vals)] = vals

    # 2. Visualización
    fig, ax = plt.subplots(figsize=(20, 16))
    im = ax.imshow(matriz, cmap="viridis", aspect="auto", origin="lower", interpolation='none')
    
    # Marcas de muerte (si estamos en modo permanente)
    if usar_permanente:
        for i, k in enumerate(claves):
            vals = np.array(memoria[k])
            muertes = np.where(vals == -M)[0]
            if len(muertes) > 0:
                ax.scatter(muertes, np.full_like(muertes, i), color='red', marker='x', s=10)

    plt.colorbar(im, label="log₁₀(|θ|)" if log_scale else "θ")
    plt.title(f"{'Historial Completo' if usar_permanente else 'Columnas Activas'} - Column Generation")
    plt.show()

def reconstruir_historial_completo(gen_col, M=10000):
    """
    Crea un diccionario consolidado que simula la memoria_permanente
    detectando automáticamente cuándo una columna fue eliminada.
    """
    # 1. Identificar todas las columnas que existieron alguna vez
    todas_las_keys = set(gen_col.memoria_theta.keys()) | set(gen_col.memoria_mu.keys())
    historial_completo = {}

    for k in todas_las_keys:
        # Recuperar la serie de theta o mu
        valores = gen_col.memoria_theta.get(k, gen_col.memoria_mu.get(k, []))
        
        # Simular el registro histórico: si la serie es corta, 
        # significa que fue eliminada. Rellenamos con -M hasta el final de i.
        if len(valores) <= gen_col.i:
            padding = [ -M ] * (gen_col.i - len(valores) + 1)
            historial_completo[k] = valores + padding
        else:
            historial_completo[k] = valores
