# -*- coding: utf-8 -*-
"""
Created on Tue Dec  2 21:50:52 2025
...
# MaxAbsScaler mantiene la esparsidad
scaler = MaxAbsScaler() 
...
max_iter_gc = 30  # Máximo de iteraciones del algoritmo GC
...
# In[2]: 
import joblib
# 1. Guardar la lista completa en un solo archivo .pkl
#print("Guardando K_ini_end_2 en disco...")
#joblib.dump(K_ini_end_2, 'K_ini_end_2.pkl')
#print("✅ Guardado exitoso.")

# --- CÓMO CARGARLA DESPUÉS ---
# K_loaded = joblib.load('K_ini_end_2.pkl')
# print(f"Cargadas {len(K_loaded)} columnas.")

# In[2]: 
# -----------------------------------------------------------------------------
# PASO 1: Generación de Sets Iniciales (Puntos y Rayos)
# -----------------------------------------------------------------------------
#the sane problem has X= 1000 x 1000 so i need n_features/pedazos=1000
pedazos=int(n_features/1000)  #con 1000 falla 
#pedazos=int(n_features/10 )
# In[2]:
...
K_ini_end_2 = K_forest_2(
    ...
    keep_xi=False, 
    solapar=True,
    fixed_sample_size= 500 #1000
)
print(f"-> Puntos generados: {len(K_ini_end_2)} (Tiempo: {time.time()-t0:.2f}s)")
# In[2]: 
K_ini_end_skl_2=K_forest_fast_sklearn(
    X_train, y_train, 
    n_iters=n_iters_forest, 
    n_subspaces=pedazos,
    max_features_ratio=1/pedazos,
    fixed_sample_size=500,
    tol_pruning=1e-5, #tol para cortar post filtro 
    random_state=42,solapar=True,tol=1e-5)
...
K_ini_canonico_list = generar_K_canonico_sparse(
    ...
    n_samples=n_samples, 
    tamaño=0.1 # 5% de las features como rayos iniciales
)
...
# -----------------------------------------------------------------------------
M_box=None #usar cuando muere por primera vez
...
gen_col.ingresar_parametros(
    C=C_param,
    M=M_box,
    #K_ini_points=K_ini_end_2,          # Input de PUNTOS (se etiquetarán p_X)
    #K_ini_points=K_ini_end_skl_2, 
...
lines, starting_line_number=inspect.getsourcelines(solve_master_primal_v3)
